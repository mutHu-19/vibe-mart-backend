...
router.get('/', async (req, res) => {
  try {
    ...
    let joins = `
      ...
      LEFT JOIN subcategories pss ON pss.id = ps.subcategory_id
    `;
...
router.get('/:slug', async (req, res) => {
  try {
    ...
    const [subs] = await db.query('SELECT subcategory_id FROM product_subcategories WHERE product_id = ?', [product.id]);

    product.variants = variants;
    product.category_ids = cats.map(c => c.category_id);
    product.subcategory_ids = subs.map(s => s.subcategory_id);
...
// Helper: sync category + subcategory junction tables
async function syncCategoryLinks(conn, productId, primaryCategoryId, primarySubcategoryId, extraCategoryIds, extraSubcategoryIds) {
  ...
  const allCategoryIds = new Set([primaryCategoryId, ...(extraCategoryIds || [])].filter(Boolean).map(String));
  for (const catId of allCategoryIds) {
    await conn.query('INSERT IGNORE INTO product_categories (product_id, category_id) VALUES (?,?)', [productId, catId]);
  }

  const allSubcategoryIds = new Set([primarySubcategoryId, ...(extraSubcategoryIds || [])].filter(Boolean).map(String));
  for (const subId of allSubcategoryIds) {
    await conn.query('INSERT IGNORE INTO product_subcategories (product_id, subcategory_id) VALUES (?,?)', [productId, subId]);
  }
}

// POST /api/products — create with multi-category + multi-subcategory support
router.post('/', auth, async (req, res) => {
  const {
    category_id, subcategory_id, category_ids, subcategory_ids,
    name, description, price, compare_price, cost_price, sku, images, variants, is_featured, sort_order
  } = req.body;
...
// PUT /api/products/:id — update with multi-category + multi-subcategory support
router.put('/:id', auth, async (req, res) => {
  const {
    category_id, subcategory_id, category_ids, subcategory_ids,
    name, description, price, compare_price, cost_price, sku, images, is_active, is_featured, sort_order
  } = req.body;
  const conn = await db.getConnection();
  try {
    await conn.beginTransaction();
...
router.delete('/:id', auth, async (req, res) => {
  ...
  try {
    ...
    await conn.query('DELETE FROM product_subcategories WHERE product_id = ?', [req.params.id]);
